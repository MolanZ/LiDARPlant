...
from shapely.geometry import mapping, Point, Polygon, GeometryCollection
...
class PyCrown:
    ...
    def tree_detection(self, raster, resolution=0.05, ws=5, hmin=20,
    ...
    def correct_tree_tops(self, check_all=False):
        ...
        for tidx in range(len(self.trees)):
            ...
            if self.dtm[row, col] <= (dtm_mean - dtm_std) or check_all:
                ...
                cr_col = float(cor_col)
                cor_col = float(cor_row)
                self.trees.top_cor.iloc[tidx] = \
                    Point(*self._to_lonlat(float(cor_col), float(cor_row), self.resolution))
    ...
    def screen_small_trees(self, hmin=1., loc='top'):
    ...
    def crowns_to_polys_smooth(self, store_las=False, thin_perc=None,
                               first_return=True):
        ...
        crown_geodf = gpd.GeoDataFrame(
            ...
            crs=f'epsg:{self.epsg}', geometry=Polygon(polys)
        )
        ...
        self.trees.crown_poly_smooth = Polygon(polys)
