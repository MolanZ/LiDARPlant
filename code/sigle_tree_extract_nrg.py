...
def write_tif(img_data,output_name,proj,geoinfo):
    ...
    out_file = driver.Create(output_name,cols,rows,7,6)
...
file_name = 'code/pycrownmaster/example/data/rgbd_f.tif'
data = gdal.Open(file_name)
data_Array_nrgbre = gdal_array.LoadFile(file_name)
file_name = 'code/pycrownmaster/example/data/chm_f.tif'
data = gdal.Open(file_name)
...
with codecs.open(fn, 'r','utf-8') as csvfile:
    ...
    for row in reader:
        ...
        output_file = 'data/03_08/'+str(row[11])
        if not os.path.exists(output_file):
            os.mkdir(output_file)
        output_file = 'data/03_08/'+str(row[11])+'/'+str(row[12])
        if not os.path.exists(output_file):
            os.mkdir(output_file)
        output_file = 'data/03_08/'+str(row[11])+'/'+str(row[12])+'/'+str(row[13])
        if not os.path.exists(output_file):
            os.mkdir(output_file)
